...
def active_learning():
    ...
    for iteration in range(1, total_iterations + 1):
        if rank == 0:
            with open(structures_pkl_file, "rb") as f:
                structured_data = pickle.load(f)  # Load as dictionary {serial_number: {data}}
            if iteration == 1:
                all_serials = list(structured_data.keys())
                num_samples = min(add_structures, len(all_serials))
                selected_serials = random.sample(all_serials, num_samples)
            else:
                ...
                top_20_structures=[s for s in sorted_structures if s not in previous_selected_serials][:20]
                selected_serials = top_20_structures

            selected_structures = [structured_data[serial]["structure"] for serial in selected_serials]
            previous_selected_serials.update(selected_serials)
            # Save selected serial numbers to log file
            with open(al_log_file, "a") as f:
                f.write(f"\nIteration: {iteration}:\n")
                f.write("\n".join(map(str, selected_serials)))

        else:
            selected_serials, selected_structures = None, None
        ...
        if rank < committee_size:
            ...
            with open(input_data_file, "a") as f:
        ...
        if structure_disagreements:
            if rank == 0:
                for conf_id, disagreements in structure_disagreements.items():
